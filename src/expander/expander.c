...
char	*expand_dollar(char *arg, t_status *exit)
{
	char	*expansion;
	char	*sub;
	char	*p;
...
char	*expand_token(char *arg, t_status *exit);

t_list	*expand_double_quote_node(char *arg, size_t *i, t_status *exit)
{
	t_list	*node;
	char	*expansion;
	char	*buf;
	...
	*buf = '\0';
	(*i) = buf - arg + 1;
	expansion = expand_token(arg + 1, exit);
	if (!expansion)
		return (NULL);
	node = ft_lstnew(expansion);
	return (node);
...
t_list	*expand_dollar_node(char *arg, size_t *i, t_status *exit)
{
	t_list	*node;
	char	*expansion;

	expansion = expand_dollar(arg, exit);
	if (!expansion)
		return (NULL);
	(*i)++;
	skip_until_quote_or_dollar(arg, i);
